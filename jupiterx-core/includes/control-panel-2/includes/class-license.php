...
class JupiterX_Core_Control_Panel_License
{
	...
	public function register_license()
	{
		if (! check_ajax_referer(self::NONCE_ACTION, 'nonce') && ! isset($_POST['mode'])) {
			wp_send_json_error([
				'code'    => 'nonce_error',
				'message' => __('Action is not allowed.', 'jupiterx-core'),
			]);
		}
		...
		wp_send_json_success([
			...
			'message' => __('You have successfully registered.', 'jupiterx-core'),
		]);
	}
	...
	public function verify_api_key($api_key)
	{
		...
		if (
			...
			return new WP_Error(
				...
				sprintf(__('Received an error while retrieving details for your API key. Please contact <a href="%s" target="_blank">Artbees support</a>.', 'jupiterx-core'), 'https://my.artbees.net/create-ticket/')
			);
		}
	...
	public function verify_purchase_code($email, $purchase_code)
	{
		...
		if (! $purchase_code || $this->is_sha256($purchase_code)) {
			return new WP_Error(
				'purchase_code_error',
				__('Purchase code is invalid.', 'jupiterx-core')
			);
		}
		...
		if (! is_email($email)) {
			return new WP_Error(
				...
				__('Please use a valid email address.', 'jupiterx-core')
			);
		}
		...
		if (count($result['output']) < 4) {
			return new WP_Error(
				...
				// translators: %s Artbees support link
				sprintf(__('Received an error while retrieving details for your purchase code. Please contact <a href="%s" target="_blank">Artbees support</a>.', 'jupiterx-core'), 'https://my.artbees.net/create-ticket/')
			);
		}
	...
	public function revoke_license()
	{
		...
		if (! empty($data['api_key']) && empty($data['access_token'])) {
			...
			wp_send_json_success([
				'code'    => 'revoke_success',
				'message' => __('Your license has been successfully revoked.', 'jupiterx-core'),
			]);
		}
