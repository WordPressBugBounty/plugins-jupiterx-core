...
if (! class_exists('JupiterX_Core_Control_Panel_Install_Template')) {
	class JupiterX_Core_Control_Panel_Install_Template
	{
		...
		public function installRequiredPlugins($template_name)
		{
			...
			$multi_site_message    = 'Template installation is only allowed for user with Super Admin role. Please contact your website\'s administrator. <a target="_blank" href="https://help.jupiterx.com/article/33-how-to-import-ready-made-website-demos-with-jupiterx">Learn More</a>';
		...
		public function resetWordpressDatabase($tables = array(), $exclude_tables = array(), $install_needed = false)
		{
			...
			if ($install_needed) {
				...
				if (is_array($exclude_tables) && count($exclude_tables) > 0) {
					array_walk(
						$exclude_tables,
						function (&$ex_value, $key) use ($wpdb) {
...
add_filter(
	'pre_transient__wc_activation_redirect',
	function () {
		return 0;
	}
);
...
add_filter(
	'pre_transient__vc_page_welcome_redirect',
	function () {
		return 0;
	}
);
