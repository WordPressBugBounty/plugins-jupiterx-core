...
?>
		<tr class="plugin-update-tr">
			<td colspan="<?php echo esc_attr($table->get_column_count()); ?>" class="plugin-update colspanchange">
...
<?php
	...
	public function check_for_update($checked_data)
	{
		...
		if ($transient_array) {
			...
			$response['url']         = 'https://my.artbees.net/change-log/';
	...
	public function jupiterx_modify_auto_update()
	{
		...
		if (false === $package_url) {
			$error = __('Something went wrong!', 'jupiterx-core');
	...
	public function jupiterx_get_theme_release_package_url()
	{
		...
		if (empty($api_key)) {
			wp_send_json_error(esc_html__('API Key is missing.', 'jupiterx-core'));
		}
